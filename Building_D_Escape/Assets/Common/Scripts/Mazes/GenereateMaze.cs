...
// Создаем экземпляр контейнера
         GameObject container = GameObject.FindWithTag("ContainerMaze"); // Или используйте GameObject.Find("ИмяКонтейнера")
    ...
    for(int i = 0; i < numX; ++i)
    {
      for(int j = 0; j < numY; ++j)
      {
        ...
        rooms[i, j].transform.SetParent(container.transform, false);
      }
    }
